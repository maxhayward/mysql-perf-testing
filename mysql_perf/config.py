...
def _load_private_key(path: str, passphrase: str | None) -> bytes:
    """Load a private key and return PKCS8 DER bytes (what Snowflake wants).

    Accepts the common formats so a key generated by openssl *or* ssh-keygen
    works: PEM (PKCS#1/PKCS#8), OpenSSH, or raw DER.
    """
    ...
    for loader in (
        ...
        try:
            key = loader(data, password=pw)
            break
        except Exception as exc:  # try the next format
            errors.append(f"{loader.__name__}: {exc}")
    else:
        raise ValueError(
            f"could not load private key {path!r} as PEM, OpenSSH, or DER:\n  "
            + "\n  ".join(errors)
        )
